...
def create_paginated_response(
    items: List[Any], total: int, page: int, size: int, base_url: str
) -> Page:
    ...
    links = {
        ...
        "next": (
            f"{base_url}?page={page + 1}&size={size}" if (page * size) < total else None
        ),
        "prev": f"{base_url}?page={page - 1}&size={size}" if page > 1 else None,
    }
    return Page(items=items, total=total, page=page, size=size, links=links)
