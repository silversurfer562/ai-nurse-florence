...
def schedule_task(
    ...
    **kwargs: Any,
    ...
    background_tasks.add_task(
        _run_task_with_status_tracking, task_id, task_func, *args, **kwargs
    )
...
async def _run_task_with_status_tracking(
    task_id: str, task_func: Callable, *args: Any, **kwargs: Any
) -> None:
    """
            ...
            async with session.post(callback_url, json=payload) as response:
                if response.status >= 400:
                    logger.error(
                        ...
                        extra={"task_id": task_id, "status_code": response.status},
                    )
                else:
                    logger.info(
