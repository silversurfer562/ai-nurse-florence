...
from src.models.disease_reference import DiseaseReference, DiagnosisPromotionQueue


router = APIRouter(prefix="/api/v1/disease-reference", tags=["Disease Reference"])
...
async def search_disease_reference(
    q: str = Query(..., min_length=2, description="Search query (disease name or ICD-10 code)"),
    ...
    **Example:**
    ...
    search_filter = (
        DiseaseReference.disease_name.ilike(f"%{q}%") |
...
async def get_disease_by_icd10(
    icd10_code: str,
    db: Session = Depends(get_db)
):
    """
    ...
    **Example:**
    ...
    ```
    """
    disease = db.query(DiseaseReference).filter(
        DiseaseReference.icd10_codes.like(f"%{icd10_code}%")
    ).first()

    if not disease:
        raise HTTPException(
            status_code=404,
            detail=f"No reference found for ICD-10 code: {icd10_code}"
        )
...
async def get_database_stats(db: Session = Depends(get_db)):
    ...
    **Returns:**
    ...
    ).group_by(DiseaseReference.disease_category).all()
    ...
    return {
        ...
        "most_searched": [
            ...
            for name, count in top_searched if count > 0
        ]
    }
...
async def request_promotion_to_full_library(
    ...
    **Example:**
    ...
    if not disease:
        raise HTTPException(status_code=404, detail="Disease not found in reference database")
    ...
    existing_request = db.query(DiagnosisPromotionQueue).filter_by(
