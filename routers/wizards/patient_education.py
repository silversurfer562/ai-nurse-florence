...
class Step1Input(BaseModel):
    topic: str = Field(
        ...,
        description="The medical condition or topic for the handout.",
...
class Step2Input(BaseModel):
    ...
    selected_sections: List[str] = Field(
        ..., description="Sections to include in the handout."
    )
    custom_sections: Optional[List[str]] = Field(
        None, description="User-defined custom sections."
    )
    target_audience: str = Field(
        "patient", description="Audience (e.g., 'patient', 'caregiver', 'child')."
    )
    reading_level: str = Field(
        "8th grade",
        description="Desired reading level (e.g., '5th grade', '8th grade', 'high school').",
    )
...
@router.post(
    ...
    summary="Start the patient education wizard",
)
...
@router.post(
    "/step2-generate", response_model=Step2Response, summary="Generate the handout"
)
async def generate_handout(step2_input: Step2Input):
    ...
    **Step 2: Customize and Generate**
    ...
    try:
        ...
        logger.error(
            f"Handout generation failed for task {task_id}: {e}", exc_info=True
        )
...
@router.get(
    ...
    summary="Get the generated handout",
)
async def get_generation_result(task_id: str):
    ...
    **Step 3: Retrieve the Result**
    ...
    if not result:
        return create_error_response(
            "Generation task not found.", status.HTTP_404_NOT_FOUND, "task_not_found"
        )
...
async def _generate_handout_content(wizard_id: str, task_id: str):
    ...
    sections = session_data["selected_sections"] + (
        session_data.get("custom_sections") or []
    )
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert in creating patient-friendly medical education materials.",
                },
        ...
        generation_results[task_id] = GenerationResult(
            status="completed", handout_text=handout_text
        )
        ...
        generation_results[task_id] = GenerationResult(
            status="failed", error="Failed to generate content from AI model."
        )
        raise
