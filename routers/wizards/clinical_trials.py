...
class Step1Input(BaseModel):
    ...
    condition: str = Field(
        ...,
        description="The medical condition to search trials for.",
...
class Step2Input(BaseModel):
    ...
    intervention: Optional[str] = Field(
        None, description="Specific drug, device, or treatment.", example="Osimertinib"
    )
    location: Optional[str] = Field(
        None,
        description="Geographic location (e.g., city, state, country).",
...
@router.post(
    ...
    summary="Step 1: Start the clinical trial search wizard",
)
...
async def add_condition(step1_input: Step1Input):
    ...
    logger.info(
        f"Updated trial search session {wizard_id} with condition: {step1_input.condition}"
    )
...
@router.post(
    ...
    summary="Step 3: Add optional filters and search",
)
async def search_trials(step2_input: Step2Input):
    ...
    return create_success_response(
        {"search_summary": search_summary, "results": mock_results}
    )
