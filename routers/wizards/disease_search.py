...
class StartWizardInput(BaseModel):
    topic: str = Field(
        ...,
        description="The medical condition or topic for the report.",
...
class GenerateReportInput(BaseModel):
    ...
    age_group: Optional[str] = Field(
        None,
        description="Patient age group (e.g., 'Pediatric', 'Adult', 'Geriatric').",
        example="Geriatric",
    )
    comorbidities: Optional[List[str]] = Field(
        None,
        description="List of relevant patient comorbidities.",
        example=["Hypertension", "Chronic Kidney Disease"],
    )
...
@router.post(
    "/start",
    response_model=StartWizardResponse,
    summary="Step 1: Start the Disease Report Wizard",
)
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an AI assistant that helps structure clinical reports. Respond in JSON format.",
                },
        ...
        logger.info(
            f"Started disease report wizard session: {wizard_id} for topic: {topic}"
        )
        ...
        logger.error(
            f"Failed to get dynamic suggestions for topic '{topic}': {e}", exc_info=True
        )
...
@router.post(
    ...
    summary="Step 2: Generate the Custom Report",
)
async def generate_report(step_input: GenerateReportInput):
    ...
    if step_input.age_group:
        prompt_parts.append(
            f"Tailor the information for a '{step_input.age_group}' patient."
        )

    if step_input.comorbidities:
        comorbidities_str = ", ".join(step_input.comorbidities)
        prompt_parts.append(
            f"Pay special attention to interactions and considerations related to these comorbidities: {comorbidities_str}."
        )
    ...
    try:
        ...
        report_response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a clinical information specialist AI, creating detailed reports for healthcare professionals.",
                },
                {"role": "user", "content": prompt},
            ],
        )
        ...
        next_steps_response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an AI assistant that suggests relevant follow-up actions. Respond in JSON format.",
                },
        ...
        logger.error(
            f"Disease report generation failed for session {wizard_id}: {e}",
            exc_info=True,
        )
        return create_error_response(
