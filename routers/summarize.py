...
async def chat_endpoint(payload: Dict[str, Any], request: Request):
    ...
    if not prompt:
        logger.warning("Missing prompt in request", extra={"request_id": request_id})
...
async def chat_endpoint_async(
    ...
    return create_success_response(
        data=task_info, links={"status": f"/api/v1/summarize/tasks/{task.id}"}
    )
...
async def get_task(task_id: str):
    ...
    if (
        ...
        and task_result.status != "PENDING"
        and task_result.status != "STARTED"
    ):
        ...
        if backend is None or result is None:
            return create_error_response(
                f"Task {task_id} not found", status.HTTP_404_NOT_FOUND, "task_not_found"
            )
