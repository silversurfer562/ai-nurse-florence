...
MODEL_SELECTION_MATRIX: Dict[str, Dict[str, str]] = {
    ...
    "drug_lookup": {
        "basic": CURRENT_MODEL_MAP["gpt4_basic"],
        "advanced": CURRENT_MODEL_MAP[
            "gpt4_advanced"
        ],  # Drug interactions need reasoning
    ...
    "symptom_analysis": {
        "basic": CURRENT_MODEL_MAP[
            "gpt4_advanced"
        ],  # All symptom analysis needs reasoning
    ...
    "wizard_treatment_plan": {
        "basic": CURRENT_MODEL_MAP[
            "gpt4_advanced"
        ],  # Treatment planning needs reasoning
        "advanced": CURRENT_MODEL_MAP["gpt4_advanced"],
        "premium": CURRENT_MODEL_MAP["gpt5_premium"],
    },
    "wizard_sbar": {
...
ENTERPRISE_SELECTION_MATRIX: Dict[str, Dict[str, str]] = {
    ...
    "clinical_reasoning": {
        "basic": ENTERPRISE_MODEL_MAP[
            "enterprise_omni"
        ],  # Always use best for clinical reasoning
        "advanced": ENTERPRISE_MODEL_MAP["enterprise_omni"],
        "premium": ENTERPRISE_MODEL_MAP["enterprise_omni"],
    },
    "chatgpt_store_query": {
...
def select_model_for_context(
    ...
    logger.info(
        f"Selecting model for context: {context}, complexity: {complexity}, mode: {mode.value}"
    )
    ...
    if mode in [DeploymentMode.ENTERPRISE_CHATGPT, DeploymentMode.CHATGPT_STORE]:
        context_models = ENTERPRISE_SELECTION_MATRIX.get(
            context, ENTERPRISE_SELECTION_MATRIX.get("medical_summary", {})
        )
        selected_model = context_models.get(
            complexity, ENTERPRISE_MODEL_MAP["enterprise_standard"]
        )
    ...
    # Production API model selection (your existing strategy)
        ...
        context_models = MODEL_SELECTION_MATRIX.get(
            context, MODEL_SELECTION_MATRIX.get("medical_summary", {})
        )
...
def get_enterprise_model(task_type: str) -> str:
    ...
    if mode == DeploymentMode.ENTERPRISE_CHATGPT:
        ...
        enterprise_task_map = {
            ...
            "symptom_analysis": ENTERPRISE_MODEL_MAP[
                "enterprise_omni"
            ],  # Best model for clinical analysis
            "clinical_reasoning": ENTERPRISE_MODEL_MAP["enterprise_omni"],
            "treatment_planning": ENTERPRISE_MODEL_MAP["enterprise_omni"],
            "drug_interactions": ENTERPRISE_MODEL_MAP[
                "enterprise_omni"
            ],  # Safety-critical
            "patient_education": ENTERPRISE_MODEL_MAP["enterprise_turbo"],
            "literature_summary": ENTERPRISE_MODEL_MAP["enterprise_standard"],
            "chatgpt_interaction": ENTERPRISE_MODEL_MAP[
                "enterprise_turbo"
            ],  # For ChatGPT Store conversations
        }

        selected = enterprise_task_map.get(
            task_type, ENTERPRISE_MODEL_MAP["enterprise_standard"]
        )
        ...
        store_task_map = {
            ...
            "symptom_analysis": ENTERPRISE_MODEL_MAP[
                "enterprise_turbo"
            ],  # Good balance for store users
            "clinical_reasoning": ENTERPRISE_MODEL_MAP["enterprise_turbo"],
            "patient_education": ENTERPRISE_MODEL_MAP["enterprise_standard"],
            "gpt_conversation": ENTERPRISE_MODEL_MAP[
                "enterprise_turbo"
            ],  # Interactive conversations
            "user_query": ENTERPRISE_MODEL_MAP[
                "enterprise_standard"
            ],  # Basic user queries
        }

        selected = store_task_map.get(
            task_type, ENTERPRISE_MODEL_MAP["enterprise_standard"]
        )
...
def validate_model_for_task(context: str, model_name: str) -> bool:
    ...
    if context in safety_critical_contexts:
        return capabilities.get("tier") in [
            "advanced",
            "premium",
            "enterprise_advanced",
            "enterprise_omni",
...
def validate_enterprise_config() -> Dict[str, bool]:
    ...
    return {
        "enterprise_mode_enabled": get_deployment_mode()
        == DeploymentMode.ENTERPRISE_CHATGPT,
        "chatgpt_store_mode_enabled": get_deployment_mode()
        == DeploymentMode.CHATGPT_STORE,
