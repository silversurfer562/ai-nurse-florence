...
class BaseClinicalService(ABC):
    ...
    def _log_query(self, query: str, **kwargs):
        ...
        logger.info(
            f"{self.service_name}: Processing query '{query}' with params {kwargs}"
        )
...
class ServiceRegistry:
    ...
    def get_services_by_tag(self, tag: str) -> List[BaseClinicalService]:
        ...
        return sorted(
            services,
            key=lambda s: self._service_metadata[s.service_name]["priority"],
            reverse=True,
        )
    ...
    def list_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: {"service": service, "metadata": self._service_metadata[name]}
