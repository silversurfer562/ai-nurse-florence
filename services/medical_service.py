...
class MedicalDataService:
    ...
    async def lookup_disease_info(self, term: str) -> Dict[str, Any]:
        ...
        if _has_prompt_enhancement:
            effective_term, needs_clarification, clarification_question = (
                enhance_prompt(term, "disease_lookup")
            )
        else:
            effective_term = term
        ...
        if LIVE:
            try:
                ...
                if self.openai_client and result.get("data"):
                    enhanced_result = await self._enhance_with_ai_summary(
                        result, effective_term
                    )
    ...
    async def lookup_drug_info(self, drug_name: str) -> Dict[str, Any]:
        ...
        if _has_prompt_enhancement:
            effective_term, needs_clarification, clarification_question = (
                enhance_prompt(drug_name, "drug_lookup")
            )
        else:
    ...
    async def lookup_symptoms(self, symptoms: str) -> Dict[str, Any]:
        ...
        if LIVE and self.openai_client:
            try:
                ...
                return await self._create_symptom_stub_response(
                    symptoms, educational_warning
                )
    ...
    async def _fetch_from_mydisease(self, term: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ExternalServiceException(
                "MyDisease.info API request failed", "mydisease_api"
            )
        except Exception as e:
    ...
    async def _fetch_drug_from_mydisease(self, drug_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ExternalServiceException(
                "Drug information lookup failed", "mydisease_drug_api"
            )

    async def _enhance_with_ai_summary(
        self, result: Dict[str, Any], term: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if ENTERPRISE_MODE or CHATGPT_STORE_MODE:
                model = (
                    get_enterprise_model("medical_summary")
                    if _has_model_selector
                    else "gpt-4"
                )
            else:
                model = (
                    select_model_for_context("medical_summary", "basic")
                    if _has_model_selector
    ...
    async def _ai_symptom_analysis(self, symptoms: str) -> Dict[str, Any]:
        ...
        if ENTERPRISE_MODE or CHATGPT_STORE_MODE:
            model = (
                get_enterprise_model("symptom_analysis")
                if _has_model_selector
                else "gpt-4"
            )
        else:
            model = (
                select_model_for_context("symptom_analysis", "advanced")
                if _has_model_selector
                else "gpt-4-turbo-preview"
            )
    ...
    def _format_mydisease_response(
        ...
        return {"banner": EDU_BANNER, "data": formatted_data, "success": True}

    def _format_drug_response(
        self, data: Dict[str, Any], drug_name: str
    ) -> Dict[str, Any]:
                    ...
                    formatted_drugs.append(
                        {
                            ...
                            "external_vocabulary": chemical.get(
                                "external_vocabulary", []
                            ),
                        }
                    )
        ...
        return {
            "banner": EDU_BANNER,
            "query": drug_name,
            "data": {"drugs": formatted_drugs, "source": "MyDisease.info PharmGKB"},
            "success": True,
            "timestamp": datetime.utcnow().isoformat(),
        }

    async def _create_stub_response(
        self, term: str, no_results: bool = False
    ) -> Dict[str, Any]:
    ...
    async def _create_symptom_stub_response(
        self, symptoms: str, warning: str
    ) -> Dict[str, Any]:
