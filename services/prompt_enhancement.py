...
def enhance_prompt(
    prompt: str, service_type: str = "general"
    ...
    if len(normalized_prompt.split()) <= 2:
        ...
        if not any(condition in normalized_prompt for condition in COMMON_CONDITIONS):
            ...
            return (
                prompt,
                True,
                "Could you provide more details about your medical question?",
            )
...
def enhance_summarize_prompt(prompt: str) -> Tuple[str, bool]:
    ...
    for condition in COMMON_CONDITIONS:
        if condition in prompt and "summar" not in prompt:
            return (
                f"Summarize key clinical information about {condition} for healthcare professionals",
                True,
            )
...
def enhance_education_prompt(prompt: str) -> Tuple[str, bool]:
    ...
    for condition in COMMON_CONDITIONS:
        if prompt == condition or prompt == f"{condition}?":
            return (
                f"Provide patient-friendly information about {condition} including symptoms, treatments, and self-care tips",
                True,
            )
    ...
    if len(prompt.split()) <= 2:
        for condition in COMMON_CONDITIONS:
            if condition in prompt:
                return (
                    f"Provide clinical information about {condition} for healthcare professionals",
                    True,
                )
