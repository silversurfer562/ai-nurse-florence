...
class DocumentDraftingSystem:
    ...
    def compare_versions(
        self, draft_id: str, revision_1: str, revision_2: str
    ) -> Dict[str, Any]:
    ...
    def export_document(
        self, draft_id: str, format_type: Literal["text", "html", "json"] = "text"
    ...
    def get_statistics(self, created_by: Optional[str] = None) -> Dict[str, Any]:
        ...
        for draft in all_drafts:
            status_counts[draft.status.value] = (
                status_counts.get(draft.status.value, 0) + 1
            )
            type_counts[draft.document_type] = (
                type_counts.get(draft.document_type, 0) + 1
            )
        ...
        return {
            ...
            "active_drafts": len(
                [d for d in all_drafts if d.status == DocumentStatus.DRAFT]
            ),
        }
