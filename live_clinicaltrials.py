...
def search_studies(query: str, max_results: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for study in studies:
            ...
            brief_summary = identification_module.get(
            ...
            if interventions_module and "interventions" in interventions_module:
                interventions = [
                    i.get("name", "Unknown")
                    for i in interventions_module["interventions"]
                ]
            ...
            primary_outcome = (
                primary_outcomes[0].get("measure", "Unknown")
                if primary_outcomes
                else "Unknown"
            )
...
def get_study_details(nct_id: str) -> Dict[str, Any]:
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/studies/{nct_id}", params={"format": "json"}, timeout=10
        )
        response.raise_for_status()
        ...
        return {
            ...
            "summary": identification_module.get(
                "briefSummary", "No summary available"
            ),
            "url": f"https://clinicaltrials.gov/study/{nct_id}",
            "raw_data": study,
