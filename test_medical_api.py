        ...
        try:
            ...
            for service_name, description in service_tests:
                ...
                if service:
                    ...
                    print(
                        f"⚠️  {service_name.title()} service: Graceful degradation (conditional imports working)"
                    )
    ...
    async def _inner():
        ...
        try:
            ...
            if disease_service:
                ...
                print(
                    f"✅ Disease lookup successful: {result.get('disease_name', 'Unknown')}"
                )
                print(f"✅ Educational banner present: {'banner' in result}")
                print(f"✅ Symptoms count: {len(result.get('symptoms', []))}")
                print(
                    "✅ Following API Design Standards: Educational disclaimer included"
                )
            else:
                print(
                    "⚠️  Disease service: Using graceful degradation (conditional imports)"
                )
        except Exception as e:
        ...
        try:
            ...
            if pubmed_service:
                ...
                result = await pubmed_service.search_literature(
                    "nursing interventions diabetes", limit=3
                )
                print(
                    f"✅ Literature search successful: {len(result.get('articles', []))} articles"
                )
                print(f"✅ Query: {result.get('query', 'Unknown')}")
                print(f"✅ Educational banner: {'banner' in result}")
                print(
                    "✅ Following API Design Standards: Comprehensive medical references"
                )
            else:
                print(
                    "⚠️  PubMed service: Using graceful degradation (conditional imports)"
                )
        except Exception as e:
        ...
        try:
            ...
            if trials_service:
                ...
                result = await trials_service.search_trials(
                    "diabetes management", limit=2
                )
                print(
                    f"✅ Clinical trials search: {len(result.get('trials', []))} trials found"
                )
                print(
                    f"✅ Educational stub note: {result.get('service_note', 'None')[:50]}..."
                )
                print(
                    "✅ Following API Design Standards: Educational disclaimers on trial data"
                )
            else:
                print(
                    "⚠️  Clinical trials service: Using graceful degradation (conditional imports)"
                )
        except Exception as e:
...
def test_api_design_standards():
    ...
    async def _inner():
        ...
        try:
            ...
            print(
                "✅ Following API Design Standards: Type-safe request/response models"
            )
        ...
        try:
            ...
            print(
                "✅ Healthcare security: Comprehensive security headers for medical data"
            )
