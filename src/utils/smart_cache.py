...
class SmartCacheManager:
    ...
    def __init__(self):
        ...
        self.common_medical_terms = [
            ...
            "heart failure", "stroke", "infection", "sepsis", "copd"
        ]
    ...
    def _normalize_medical_query(self, query: str) -> str:
        ...
        normalized = re.sub(r'\b(acute|chronic|mild|severe|stage\s+\d+)\b', '', normalized)
    ...
    def _generate_smart_cache_key(self, 
                                 strategy: CacheStrategy,
                                 query: str,
                                 **kwargs) -> str:
        ...
        if strategy in [CacheStrategy.MEDICAL_REFERENCE, CacheStrategy.LITERATURE_SEARCH]:
    ...
    async def smart_cache_get(self,
                             strategy: CacheStrategy,
                             query: str,
        ...
        try:
            ...
            if similarity_check and strategy in [CacheStrategy.MEDICAL_REFERENCE, CacheStrategy.LITERATURE_SEARCH]:
                similar_result = await self._find_similar_cached_result(strategy, query, kwargs)
                if similar_result:
                    self._record_cache_metrics(cache_key, strategy, True, start_time, note="similarity_match")
                    if _has_metrics:
    ...
    async def smart_cache_set(self,
                             strategy: CacheStrategy,
                             query: str,
                             result: Any,
                             **kwargs) -> bool:
        ...
        try:
            ...
            # Set with strategy-specific TTL
            success = await cache_set(cache_key, cache_data, config.ttl_seconds)
            
            if success:
                logger.debug(f"Smart cache set: {cache_key} (strategy: {strategy.value}, TTL: {config.ttl_seconds}s)")
    ...
    async def _find_similar_cached_result(self,
                                        strategy: CacheStrategy,
                                        query: str,
        ...
        try:
            ...
            for similar_term in similar_terms:
                similar_key = self._generate_smart_cache_key(strategy, similar_term, **kwargs)
                result = await cache_get(similar_key)
                if result:
                    logger.debug(f"Found similar cache result: {similar_term} for {query}")
                    return result
    ...
    def _record_cache_metrics(self,
                             cache_key: str,
                             strategy: CacheStrategy,
                             hit: bool,
                             start_time: datetime,
    ...
    async def warm_cache_for_common_queries(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Cache warming completed for {len(self.common_medical_terms)} common terms")
        ...
        try:
            ...
            for strategy in CacheStrategy:
                ...
                if strategy_metrics:
                    ...
                    avg_response_time = sum(m.response_time_ms for m in strategy_metrics) / len(strategy_metrics)
                    
                    stats_by_strategy[strategy.value] = {
                        "total_requests": len(strategy_metrics),
                        "cache_hits": hits,
                        "hit_rate": hits / len(strategy_metrics) if strategy_metrics else 0,
            ...
            return {
                "overall_statistics": {
                    ...
                    "metrics_collected_since": self.metrics_history[0].timestamp.isoformat() if self.metrics_history else None
                },
