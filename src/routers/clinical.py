...
async def optimize_query(body: OptimizeRequest) -> OptimizeResponse:
    ...
    try:
        ...
        return OptimizeResponse(
            status="success", optimization_score=score, enhanced_prompt=enhanced
        )
...
async def generate_response(body: GenerateRequest) -> GenerateResponse:
    ...
    try:
        ...
        result = await service.generate_response(
            body.prompt, context="clinical_generate"
        )
        ...
        if not answer:
            ...
            fallback = (
                result.get("error")
                if isinstance(result, dict)
                else "AI generation returned no content"
            )
            return GenerateResponse(
                status="error",
                answer=str(fallback or "AI generation returned no content"),
            )
        ...
        return GenerateResponse(
            status="error", answer=f"AI generation failed: {str(e)}"
        )
