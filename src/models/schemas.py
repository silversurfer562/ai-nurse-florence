...
class ClinicalDecisionRequest(BaseModel):
    ...
    patient_condition: str = Field(
        default=..., description="Primary patient condition or diagnosis"
    )
    severity: SeverityLevel = Field(default=SeverityLevel.MODERATE)
    comorbidities: List[str] = Field(
        default_factory=list, description="Relevant comorbidities"
    )
    care_setting: CareSetting = Field(default=CareSetting.MED_SURG)
    additional_context: Optional[str] = Field(
        default=None, description="Additional clinical context"
    )
...
class ClinicalDecisionResponse(BaseResponse):
    ...
    nursing_interventions: str = Field(
        default=..., description="Evidence-based nursing interventions"
    )
...
class RiskAssessmentRequest(BaseModel):
    ...
    assessment_type: str = Field(
        default=..., description="Type of risk assessment (morse, braden, mews)"
    )
    patient_data: Dict[str, Any] = Field(
        default=..., description="Patient assessment data"
    )
    care_setting: CareSetting = Field(default=CareSetting.MED_SURG)
...
class RiskAssessmentResponse(BaseResponse):
    ...
    risk_score: int = Field(default=..., description="Calculated risk score")
    risk_level: str = Field(default=..., description="Risk level interpretation")
    interventions: List[str] = Field(
        default=..., description="Recommended interventions"
    )
    reassessment_interval: str = Field(default=..., description="When to reassess")
...
class SBARRequest(BaseModel):
    ...
    situation: str = Field(default=..., description="Current patient situation")
    background: str = Field(default=..., description="Relevant background information")
    assessment: str = Field(default=..., description="Clinical assessment findings")
...
class SBARResponse(BaseResponse):
    ...
    formatted_report: str = Field(default=..., description="Formatted SBAR report")
    clinical_priorities: List[str] = Field(default_factory=list)
...
class LiteratureSearchRequest(BaseModel):
    ...
    query: str = Field(default=..., min_length=3, description="Search query")
    max_results: int = Field(default=10, ge=1, le=50)
    filter_years: Optional[int] = Field(default=None, ge=1990, le=2025)
    study_types: List[str] = Field(
        default_factory=list, description="Preferred study types"
    )
...
class PatientEducationRequest(BaseModel):
    ...
    topic: str = Field(default=..., description="Education topic")
...
class PatientEducationResponse(BaseResponse):
    ...
    content: str = Field(default=..., description="Educational content")
...
class WizardStepRequest(BaseModel):
    ...
    wizard_type: str = Field(default=..., description="Type of wizard workflow")
    step_data: Dict[str, Any] = Field(default=..., description="Data for current step")
...
class SBARStep(BaseModel):
    ...
    step_number: int = Field(default=..., ge=1, le=4, description="Step number (1-4)")
    step_name: str = Field(
        ...,
        description="SBAR step name (Situation, Background, Assessment, Recommendation)",
    )
    title: str = Field(default=..., description="Step title")
    ...
    fields: List[Dict[str, Any]] = Field(
        default_factory=list, description="Form fields for this step"
    )
...
class SBARWizardRequest(BaseModel):
    ...
    step_number: int = Field(default=..., ge=1, le=4, description="Current step number")
    data: Dict[str, Any] = Field(
        default=..., description="Step data collected from user"
    )
...
class WizardSession(BaseModel):
    ...
    wizard_id: str = Field(default=..., description="Unique wizard session ID")
    wizard_type: str = Field(
        default=..., description="Type of wizard (sbar, treatment_plan, etc.)"
    )
    current_step: Union[int, str] = Field(
        default=..., description="Current step number or 'complete'"
    )
    total_steps: int = Field(default=..., description="Total number of steps")
    collected_data: Dict[str, Any] = Field(
        default_factory=dict, description="Collected step data"
    )
...
class SBARWizardResponse(BaseResponse):
    ...
    wizard_session: WizardSession = Field(
        default=..., description="Current wizard session state"
    )
    current_step: Optional[SBARStep] = Field(
        default=None, description="Current step data"
    )
    progress: Dict[str, Any] = Field(
        default_factory=dict, description="Progress information"
    )
    sbar_report: Optional[Dict[str, Any]] = Field(
        default=None, description="Completed SBAR report"
    )
...
class SBARReport(BaseModel):
    ...
    banner: str = Field(default=..., description="Educational disclaimer")

    # SBAR Sections
    sections: Dict[str, Any] = Field(
        default_factory=dict, description="SBAR section data"
    )
    ...
    enhanced_content: Optional[Dict[str, Any]] = Field(
        default=None, description="AI-enhanced content"
    )
