...
class ClinicalDecisionService:
    ...
    async def get_nursing_recommendations(
        ...
        if self.openai_available:
            ...
            if "error" not in result:
                result.update(
                    {
                        "nursing_interventions": self._extract_interventions(
                            result.get("response", "")
                        ),
                        "monitoring_parameters": self._extract_monitoring(
                            result.get("response", "")
                        ),
                ...
                result.setdefault(
                    "evidence_level",
                    result.get("evidence_level", "Level VII - Expert Opinion"),
                )
                result.setdefault(
                    "safety_considerations", result.get("safety_considerations", [])
                )
            ...
            return self._create_nursing_stub(
                patient_condition, nursing_concerns, priority_level
            )
    ...
    async def assess_care_escalation(
        ...
        if self.openai_available:
            ...
            if "error" not in result:
                result.update(
                    {
                        "escalation_recommended": self._assess_escalation_need(
                            clinical_indicators
                        ),
    ...
    def _extract_interventions(self, ai_response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(
                ...
                for keyword in ["monitor", "assess", "administer", "provide", "ensure"]
            ):
    ...
    def _create_nursing_stub(
        self, condition: str, concerns: List[str], priority: str
    ) -> Dict[str, Any]:
    ...
    def _create_escalation_stub(
        self, patient_data: Dict[str, Any], indicators: List[str]
    ) -> Dict[str, Any]:
